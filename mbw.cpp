...
void thread_func(char *src, char *dst, unsigned long long block_size, unsigned long long total)
{
...
double worker(unsigned long long asize, long *a, long *b, int type, unsigned long long block_size)
{
    ...
    { /* memcpy block test */
        ...
        {
            threads[i] = std::thread(thread_func, (char *)src + array_bytes * i / thread_num, (char *)dst + array_bytes * i / thread_num, block_size, array_bytes / thread_num);
        }
        for (int i = 0; i < thread_num; i++)
        {
            threads[i].join();
